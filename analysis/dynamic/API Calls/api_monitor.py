...
CONFIG_FILE = "analysis/Dynamic/API Calls/api_config.json"
OUTPUT_FILE = "analysis/Dynamic/API Calls/api_monitor_output.json"
...
def inject_config_into_script(config):
    ...
    if original_script is None:
        print(f"[!] Error: hook_apis.js not found in any of the expected locations")
...
def main():
    ...
    try:
        ...
        print(f"[*] API calls will be logged to: {OUTPUT_FILE}")
        print("[*] Press 'e' to stop Frida monitoring")
        print("=" * 60)
                ...
                if ready:
                    ...
                    if line:
                        ...
                        if line:
                            ...
                            if not json_data:
                                # If not JSON, print as regular Frida output
                                print(f"[FRIDA] {line}")
                else:
                    time.sleep(0.1)
